...
if __name__ == '__main__':
        ...
        if start_searching != None:
            ...
            all_db_query = "SELECT symbol,name,date_YYYY,date_MM,date_DD,open_price,high_price,low_price,close_price,volume,value_traded FROM stocks"
            stock_symbol_query =  "SELECT DISTINCT symbol FROM stocks"
 
            klse_all_db_df = pandas.read_sql_query(all_db_query,conn)
            ...
            klse_all_db_df = klse_all_db_df.assign(Date="0")
            klse_all_db_df = klse_all_db_df.assign(ma20d=klse_all_db_df.close_price)
            klse_all_db_df = klse_all_db_df.assign(ma50d=klse_all_db_df.close_price)
            klse_all_db_df = klse_all_db_df.assign(Bol_upper=klse_all_db_df.close_price)
            klse_all_db_df = klse_all_db_df.assign(Bol_lower=klse_all_db_df.close_price)
            klse_all_db_df = klse_all_db_df.assign(Bol_BW=0.0)
            klse_all_db_df = klse_all_db_df.assign(Bol_BW_200MA=0.0)
            klse_all_db_df = klse_all_db_df.assign(exma20d=klse_all_db_df.close_price)
            klse_all_db_df = klse_all_db_df.assign(exma50d=klse_all_db_df.close_price)
            
            #Init date array
            klse_all_db_df_date_arr = []
            
            print("---------------- klse_all_db_df contains")
            print klse_all_db_df
            print("---------------- klse_all_db_df ends")

            print("---------------- klse_stock_symbol_df contains")
            print klse_stock_symbol_df
            print("---------------- klse_stock_symbol_df ends")
            
            #print klse_all_db_df['symbol'].size
            print klse_all_db_df['symbol'].size
            
            for i in range( klse_all_db_df['symbol'].size ):        
                try:
                    print "----------------" + klse_all_db_df['symbol'][i] + "-- construct date ----------------"
                    print(klse_all_db_df['date_YYYY'][i])
                    print(klse_all_db_df['date_MM'][i])
                    print(klse_all_db_df['date_DD'][i])
                    print(str(klse_all_db_df['date_YYYY'][i])+"/"+str(klse_all_db_df['date_MM'][i])+"/"+str(klse_all_db_df['date_DD'][i]))
                    klse_all_db_df_date_arr.append(str(klse_all_db_df['date_YYYY'][i]).zfill(4) +"/"+str(klse_all_db_df['date_MM'][i]).zfill(2) +"/"+str(klse_all_db_df['date_DD'][i]).zfill(2) )
                    klse_all_db_df.iat[i,11] = str(klse_all_db_df['date_YYYY'][i]).zfill(4) +"/"+str(klse_all_db_df['date_MM'][i]).zfill(2) +"/"+str(klse_all_db_df['date_DD'][i]).zfill(2)
                    print(klse_all_db_df_date_arr)
            ...
            for i in range( klse_all_db_df['symbol'].size ):        
                try:
                    print "----------------" + klse_all_db_df['symbol'][i] + "----------------"
                    
                    print "---------------- pt 2 ----------------"
                    klse_all_db_df['Date'] =  pandas.to_datetime( klse_all_db_df['Date'], errors='raise', dayfirst=False, yearfirst=True)
                    temp_data_set = klse_all_db_df.sort('Date',ascending = True ) #sort to calculate the rolling mean
                    
                    print "---------------- pt 3 ----------------"
                    temp_data_set['ma20d'] = pandas.rolling_mean(klse_all_db_df['close_price'], window=20)
                    temp_data_set['ma50d'] = pandas.rolling_mean(klse_all_db_df['close_price'], window=50)
                    temp_data_set['Bol_upper'] = pandas.rolling_mean(klse_all_db_df['close_price'], window=20) + 2* pandas.rolling_std(klse_all_db_df['close_price'], 20, min_periods=20)
                    temp_data_set['Bol_lower'] = pandas.rolling_mean(klse_all_db_df['close_price'], window=20) - 2* pandas.rolling_std(klse_all_db_df['close_price'], 20, min_periods=20)
                    temp_data_set['Bol_BW'] = ((klse_all_db_df['Bol_upper'] - klse_all_db_df['Bol_lower'])/klse_all_db_df['ma20d'])*100
                    temp_data_set['Bol_BW_200MA'] = pandas.rolling_mean(klse_all_db_df['Bol_BW'], window=50)#cant get the 200 daa
                    temp_data_set['Bol_BW_200MA'] = klse_all_db_df['Bol_BW_200MA'].fillna(method='backfill')##?? ,may not be good
                    temp_data_set['exma20d'] = pandas.ewma(klse_all_db_df['close_price'], span=20)
    ...
    #                print  "temp_data_set['Bol_upper'][0] is " + str( temp_data_set['Bol_upper'][0] )
                    ...
                    if True:#temp_data_set['close_price'][0] > temp_data_set['Bol_upper'][0] :
                        print "------------------------------------------------------"
                        print "---------found   " + klse_all_db_df['name'][i] + " matched-------"
                        temp_data_set.plot(x='Date', y=['close_price','ma20d','Bol_upper','Bol_lower' ])
                        plt.show()
                        print "--------------------------------------------------------"
                except TypeError as current_error:
                    print "TypeError, skip this stock " + str( klse_all_db_df['symbol'][i] + " error: " + str(current_error))
                    continue
                except KeyError as current_error:
                    print "KeyError, skip this stock" + str( klse_all_db_df['symbol'][i] + " error: " + str(current_error))
                    continue
